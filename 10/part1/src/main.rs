    ...
    for yi in 0..asteroids.len() {
        for xi in 0..asteroids[yi].len() {
            if asteroids[yi].as_bytes()[xi] as char == '#' && (x!=xi && y!=yi) {
                let slope = get_slope(x as i32,y as i32,xi as i32,yi as i32).to_string(); //Because you can't use f32 as HashMap keys.
                let dist = get_man_dist(x as i32,y as i32,xi as i32,yi as i32);
                let quad = match slope.as_ref() {
                    "inf" => get_quad(y as i32, yi as i32),
...
fn main() {
    ...
    println!("Best location for a station: {}, {}: {} asteroids", x, y, ast + 4);
}
